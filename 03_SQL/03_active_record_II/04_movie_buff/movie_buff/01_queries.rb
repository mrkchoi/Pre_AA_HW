...
def biggest_cast
  ...
  Movie.joins(:castings).select('movies.id, movies.title').group('movies.id').order('COUNT(castings.movie_id) DESC').limit(3)
end
...
def directed_by_one_of(them)
  ...
  id_list = Actor.where(name: them).select('actors.id')

  Movie.joins(:actors).where('movies.director_id IN (?)', id_list).select('DISTINCT movies.id, movies.title')
end

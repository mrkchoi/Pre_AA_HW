...
describe 'all_unique_chars?' do 
  it 'handles a string with only unique characters' do
    expect(all_unique_chars?('abcdefg')).to eq(true)
  end

  it 'handles a string with several repeating characters' do
    expect(all_unique_chars?('abbccddeeffrgg')).to eq(false)
  end
end
...
def array_sum_with_index(arr)
    output = 0
    arr.each_with_index {|el, i| output += el * i}
    output
end
...
def price_is_right(bids, actual_retail_price)
    lower_bids = bids.select {|el| el < actual_retail_price}
    lower_bids.max
end
...
def num_factors(number)
    (1..number).reduce(0) do |factor_count, el|
        if number % el == 0
            factor_count + 1
        else
            factor_count
